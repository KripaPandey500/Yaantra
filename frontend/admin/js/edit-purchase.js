...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!purchaseId) {
        showAlert('Invalid purchase ID.', 'error');
        setTimeout(() => {
    ...
    purchaseState.editingPurchaseId = purchaseId;
    await loadReferenceData();
    await populatePurchaseForm(purchaseId);
    bindFormEvents();
});
...
async function populatePurchaseForm(purchaseId) {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${purchasesApiBase}/${purchaseId}`, {
        ...
        // Set vendor
        renderVendorOptions(purchase.vendorUserId);
        
        // Set form items
        purchaseState.formItems = (purchase.purchaseDetails || []).map(detail => ({
            productId: String(detail.productId || ''),
            quantity: Number(detail.quantity || 1)
        }));

        if (!purchaseState.formItems.length) {
        ...
        console.error(error);
        showAlert('Unable to load purchase details. Redirecting...', 'error');
        setTimeout(() => {
            window.location.href = 'purchases.html';
        }, 2000);
    }
...
async function submitPurchaseForm(event) {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${purchasesApiBase}/${purchaseState.editingPurchaseId}`, {
            method: 'PUT',
            headers: { 
        ...
        if (!response.ok) {
            ...
            throw new Error(errorText || 'Failed to update purchase invoice.');
        }

        window.location.href = 'purchases.html?status=updated';
    } catch (error) {
        console.error(error);
        showAlert(error.message || 'Failed to update purchase invoice.', 'error');
    }
}
